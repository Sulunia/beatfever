	...
	if curveParams[1] == "L" or curveParams[1] == "B" or curveParams[1] == "P" then
		...
		while sliderRepeatCount >= repeatGeneration do
			if repeatGeneration % 2 ~= 0 then
				...
				--Put slider beginning point in table
				table.insert(slider, HitObject(x, y, objTime+(sliderDuration*repeatGeneration), 0.5, hitcircleGraphic))
				...
				table.insert(slider, HitObject(bx, by, (objTime +sliderDuration+sliderDuration*(repeatGeneration+1)), 0.5, hitcircleGraphic))
			end
